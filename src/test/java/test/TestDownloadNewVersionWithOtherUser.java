...
public class TestDownloadNewVersionWithOtherUser
{
	...
	public void before() throws Exception
	{
		...
		client1 = componentFactory.startClient(new Config("client1"), new Shell("client1", new TestOutputStream(System.out), new TestInputStream()));
		Thread.sleep(Util.WAIT_FOR_COMPONENT_STARTUP);
	}
	...
	public void test() throws Exception
	{
		...
		actual = client1.login("bill", "23456").toString();
		expected = "success";
		assertTrue(String.format("Response must contain '%s' but was '%s'", expected, actual), actual.contains(expected));

		actual = client1.download("upload.txt").toString();
		expected = "!data Bitte die Lehrveranstaltung im TISS bewerten. ;-)";
		assertTrue(String.format("Response must start with '%s' but was '%s'", expected, actual), actual.startsWith(expected));
		...
		actual = client1.logout().toString();
		expected = "Successfully logged out.";
		assertTrue(String.format("Response must contain '%s' but was '%s'", expected, actual), actual.contains(expected));
	}
}
